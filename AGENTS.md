...
- **Input-driven swipe handoff.** The first `SWIPE_HANDOFF_FRACTION` of horizontal drag directly collapses and docks the current detail view; the remaining distance maps immediately to ring travel. Do not add a time-based handoff or replay queue: the rendered position must stay attached to live input, including a gesture that releases during the docking portion.
